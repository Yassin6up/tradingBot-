...
import { startBotSchema, changeStrategySchema, saveApiKeysSchema, type TradingMode } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/trading-mode", async (req, res) => {
    console.log('Received request to change trading mode:', req.body);
    try {
    ...
    const { exchangeProviderManager } = await import("./services/exchange-provider-manager");
    
    // ✅ Ensure manager is initialized before switching mode
    if (!exchangeProviderManager.isReady()) {
      await exchangeProviderManager.initialize(); // Initialize with default mode
    }
    
    // Validate that the requested mode can be initialized
